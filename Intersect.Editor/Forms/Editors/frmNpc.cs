...
namespace Intersect.Editor.Forms.Editors
{
    ...
    public partial class FrmNpc : EditorForm
    {
        ...
        private void solostats_Click(object sender, EventArgs e)
        {
            double fator = 0.5;
            int level = (int)nudLevel.Value;
            double a1 = Math.Pow(level, 3);
            double a2 = Math.Pow(level, 2);
            double basexp = (50 / 3) * (a1 - 6 * a2 + 17 * level - 12);
            double xp = basexp / (2.4 * level * fator);

            nudHp.Value = (decimal)((24 * level) * 14.8747);
            nudMana.Value = (decimal)((20 * level) * 3.8747);
            nudDamage.Value = (decimal)((2.87 * level) * 1.6247);
            nudStr.Value = (decimal)((2.8 * level) * 1.3);
            nudMag.Value = (decimal)((2.8 * level) * 1.3);
            nudDef.Value = (decimal)((3.1 * level));
            nudMR.Value = (decimal)((3.1 * level));
            nudSpd.Value = 3500 + (4 * level);

            nudExp.Value = (decimal)xp;
            mEditorItem.Experience = (int)nudExp.Value;
        }

        private void lurestats_Click(object sender, EventArgs e)
        {
            double fator = 0.5;
            int level = (int)nudLevel.Value;
            double a1 = Math.Pow(level, 3);
            double a2 = Math.Pow(level, 2);
            double basexp = (50 / 3) * (a1 - 6 * a2 + 17 * level - 12);
            double xp = basexp / (4 * level * fator);


            nudHp.Value = (decimal)((14 * level) * 14.8747);
            nudMana.Value = (decimal)((12 * level) * 5.8747);
            nudDamage.Value = (decimal)((1.87 * level) * 1.6247);
            nudStr.Value = (decimal)((2.8 * level) * 1.3);
            nudMag.Value = (decimal)((2.8 * level) * 1.3);
            nudDef.Value = (decimal)((2.5 * level));
            nudMR.Value = (decimal)((2.5 * level));
            nudSpd.Value = 3500 + (3 * level);

            nudExp.Value = (decimal)xp;
            mEditorItem.Experience = (int)nudExp.Value;
        }

        private void bossstats_Click(object sender, EventArgs e)
        {
            double fator = 0.5;
            int level = (int)nudLevel.Value;
            double a1 = Math.Pow((int)nudLevel.Value, 3);
            double a2 = Math.Pow((int)nudLevel.Value, 2);
            double basexp = (50 / 3) * (a1 - 6 * a2 + 17 * level - 12);
            double xp = basexp / (10 * level * fator) / (10 * level * fator) + (10 * basexp) / (4 * level * fator);


            nudHp.Value = (decimal)((120 * (int)nudLevel.Value) * 48.8747);
            nudMana.Value = (decimal)((119 * (int)nudLevel.Value) * 15.8747);
            nudDamage.Value = (decimal)((3.87 * (int)nudLevel.Value) * 1.6247);
            nudStr.Value = (decimal)((4.2 * (int)nudLevel.Value) * 1.3);
            nudMag.Value = (decimal)((4.2 * (int)nudLevel.Value) * 1.3);
            nudDef.Value = (decimal)((4 * (int)nudLevel.Value));
            nudMR.Value = (decimal)((4 * (int)nudLevel.Value));
            nudSpd.Value = 3800 + (2 * (int)nudLevel.Value);
            nudExp.Value = (decimal)xp;
            nudTenacity.Value = 65;
            mEditorItem.Experience = (int)nudExp.Value;
        }
    }
